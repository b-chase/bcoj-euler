...
def solve(debug=False):
    ...
    res = 0
    # starting number - choose rows and columns
    for i in range(0, 20):
        check_spacings = []
        if i <= 16:
            check_spacings.append(vert)
        
        for j in range(0, 20):
            if j >=3:
                check_spacings.append(diag_left)
            if j <= 16:
                check_spacings.append(diag_right)
                check_spacings.append(horiz)
            
            start = 20*i + j
            # now for each spacing, check product of numbers
            for spacing_ct in check_spacings:
                stop = (20*i + j) + spacing_ct*4
                tmp = prod(grid[start:stop:spacing_ct])
                if debug:
                    if tmp > res:
                        print(f"New max = {tmp}")
                        print(f"starting from ({i},{j}) with step {spacing_ct}")
                        res = tmp
                else:
                    res = max(res,tmp)
